...
def sample_sequence(model, seed_seq, max_new_tokens=500, temperature=1.0):
    model.eval()
    context = seed_seq.clone().to(DEVICE)
...
def decode_tokens(tokens, sound_stream):
    with torch.no_grad():
        decoded_audio = sound_stream(tokens.unsqueeze(0).unsqueeze(-1), mode="decode")
...
if __name__ == "__main__":
    ...
    torchaudio.save(output_path, decoded_audio.unsqueeze(0), sample_rate=24000)
